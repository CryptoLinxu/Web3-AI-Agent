...
G[摘要压缩内存<br/>/lib/memory/SummaryCompressionMemory.ts]
H[滑动窗口内存<br/>/lib/memory/SlidingWindowMemory.ts]
I[配置<br/>/lib/memory/config.ts]
J[类型<br/>/lib/memory/types.ts]
end
subgraph "数据库 (supabase)"
K[初始化脚本<br/>/supabase/init.sql]
end
A --> D
A --> E
B --> A
...
- [apps/web/components/ConversationHistory.tsx:14-221](file://apps/web/components/ConversationHistory.tsx#L14-L221)
