...
## 更新摘要
**所做更改**
- 从单一数据源架构升级为双数据源故障容错架构
- 新增Binance CN和Huobi数据源支持，提供更好的数据冗余和可靠性
- 添加代理支持机制，增强网络连接的灵活性
- 实现10秒超时处理机制，提高系统的稳定性
- 简化数据结构，移除CoinGecko支持，专注于核心数据源
- 改进错误处理和故障转移策略
...
## 简介
本文件围绕 ETH 价格查询工具（getETHPrice）进行系统化技术实现说明，涵盖双数据源故障容错架构、API 调用流程、实时性保障、错误处理、API 接口设计、缓存策略、更新频率与精度控制、与其他工具的协作关系以及集成最佳实践。

根据项目 PRD，getETHPrice 属于 MVP 必做 Web3 工具之一，用于提供"可信、可溯源"的 ETH 价格数据，避免模型主观捏造链上数据。经过重大升级后，工具现已支持双数据源故障容错，显著提升了系统的可靠性和稳定性。
...
graph TB
subgraph "核心工具层"
PT["price.ts<br/>双数据源价格查询"]
TT["types.ts<br/>类型定义"]
end
subgraph "应用层"
RT["route.ts<br/>API路由与工具分发"]
PKG["package.json<br/>应用依赖配置"]
end
subgraph "数据源"
BIN["Binance CN API<br/>ETHUSDT交易对"]
...
- [route.ts:1-47](file://apps/web/app/api/tools/route.ts#L1-L47)
...
## 架构总览
getETHPrice 的调用与集成遵循"意图识别 → 工具调用 → 双数据源查询 → 结果回填 → 自然语言回复"的 Agent Loop 流程；在工具层之上，统一由 SKILL 主入口进行路由与流程控制。
...
- [route.ts:16-25](file://apps/web/app/api/tools/route.ts#L16-L25)
...
- [types.ts:11-15](file://packages/web3-tools/src/types.ts#L11-L15)
- [types.ts:30-33](file://packages/web3-tools/src/types.ts#L30-L33)
...
getETHPrice 工具经过重大升级，从单一数据源架构转变为双数据源故障容错架构，显著提升了系统的可靠性和稳定性。新架构支持 Binance CN 和 Huobi 等多个数据源，实现了自动故障转移和负载均衡。
