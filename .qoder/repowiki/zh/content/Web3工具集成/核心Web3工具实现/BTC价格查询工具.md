...
BTC价格查询工具是一个基于Next.js构建的Web3 AI Agent应用，专门用于查询比特币(BTC)和以太坊(ETH)的价格信息。该工具集成了AI对话能力和Web3链上数据查询功能，为用户提供实时的加密货币价格查询服务。

该项目采用现代化的技术栈，包括Next.js 14、React、TypeScript和Tailwind CSS，实现了从需求定义到代码交付的完整SDLC自动化流程。系统支持BTC价格查询、ETH价格查询、钱包余额查询和Gas价格查询等多种Web3相关功能。
...
Web3工具集是整个系统的核心数据层，提供了多种Web3相关的查询功能：

- **价格查询模块**: 支持BTC和ETH价格查询，包含多数据源容错机制
- **钱包余额模块**: 查询以太坊钱包地址的ETH余额
- **Gas价格模块**: 获取当前以太坊网络的Gas价格信息
- **统一类型系统**: 定义了标准的工具结果接口和数据结构

### API层

系统提供两个主要的API端点：

- **聊天API** (`/api/chat`): 支持AI对话和工具调用的完整Agent流程
- **工具API** (`/api/tools`): 直接调用特定工具的简化接口
...
- [apps/web/app/api/chat/route.ts:1-219](file://apps/web/app/api/chat/route.ts#L1-L219)
- [apps/web/app/api/tools/route.ts:1-50](file://apps/web/app/api/tools/route.ts#L1-L50)
- [packages/web3-tools/src/price.ts:1-153](file://packages/web3-tools/src/price.ts#L1-L153)

## 详细组件分析

### BTC价格查询组件

BTC价格查询功能是系统的核心特性之一，通过以下组件协同工作：
...
+parseResponse(data) BTCPriceData
}
BTCPriceQuery --> BTCPriceData : "返回"
BTCPriceQuery --> ToolResult : "封装结果"
BTCPriceQuery --> PriceSource : "使用多个数据源"
```

**图表来源**
- [packages/web3-tools/src/price.ts:89-152](file://packages/web3-tools/src/price.ts#L89-L152)
- [packages/web3-tools/src/types.ts:17-21](file://packages/web3-tools/src/types.ts#L17-L21)

#### 数据源容错机制

系统实现了多数据源容错策略，确保价格查询的可靠性：

```mermaid
sequenceDiagram
participant Client as 客户端
participant API as BTC价格API
participant Binance as Binance数据源
participant Huobi as Huobi数据源
participant Fallback as 备用方案
Client->>API : 请求BTC价格
API->>Binance : 查询BTCUSDT价格
Binance-->>API : 返回价格数据
API->>API : 解析并验证数据
API-->>Client : 返回BTC价格
Note over API,Fallback : 如果Binance失败
API->>Huobi : 查询BTCUSDT价格
Huobi-->>API : 返回价格数据
API->>API : 计算24小时涨跌
API-->>Client : 返回BTC价格
Note over API,Fallback : 如果所有数据源失败
API-->>Client : 返回错误信息
```

**图表来源**
- [packages/web3-tools/src/price.ts:91-118](file://packages/web3-tools/src/price.ts#L91-L118)
- [packages/web3-tools/src/price.ts:120-143](file://packages/web3-tools/src/price.ts#L120-L143)
...
participant ETHPrice as getETHPrice
participant BTCPrice as getBTCPrice
...
ToolsAPI->>ETHPrice : 调用getETHPrice()
ETHPrice-->>ToolsAPI : 返回ETH价格数据
ToolsAPI->>ToolsAPI : 格式化响应
ToolsAPI-->>Client : 返回JSON响应
Note over Client,ToolsAPI : 支持的工具类型
Note over ETHPrice,BTCPrice : getETHPrice()
Note over WalletBalance,GasPrice : getBTCPrice()
...
1. **实时价格查询**: 从多个可靠的数据源获取最新的BTC价格
2. **多数据源容错**: 当一个数据源不可用时自动切换到其他数据源
3. **24小时涨跌计算**: 提供BTC价格的24小时变化百分比
4. **统一数据格式**: 返回标准化的价格数据结构
5. **错误处理**: 完善的错误处理和降级机制

### 数据源配置

系统配置了两个主要的数据源来确保价格查询的可靠性：

| 数据源 | API端点 | 数据解析 | 特殊功能 |
|--------|---------|----------|----------|
| Binance CN | `/api/v3/ticker/price?symbol=BTCUSDT` | 直接价格字段 | 基础价格查询 |
| Huobi | `/market/detail/merged?symbol=btcusdt` | 包含开盘价和收盘价 | 计算24小时涨跌 |
...
```mermaid
flowchart TD
Start([BTC价格查询开始]) --> InitSources[初始化数据源列表]
InitSources --> TryBinance[尝试Binance数据源]
...
InputValid --> |是| CallTool[调用BTC价格工具]
...
BTC价格查询工具是一个功能完整、架构清晰的Web3应用。系统通过以下关键特性实现了高质量的用户体验：

### 核心优势

1. **可靠性**: 多数据源容错机制确保服务稳定性
2. **实时性**: 支持实时价格查询和快速响应
3. **可扩展性**: 模块化设计支持功能扩展
4. **用户体验**: 直观的聊天界面和流畅的交互体验
...
该工具不仅满足了当前的BTC价格查询需求，更为构建更复杂的Web3 AI应用提供了坚实的技术基础。