# 提示管理系统

<cite>
**本文档引用的文件**
- [apps/web/config/prompts.ts](file://apps/web/config/prompts.ts)
- [apps/web/components/PromptSelector.tsx](file://apps/web/components/PromptSelector.tsx)
- [apps/web/components/PromptSelectorModal.tsx](file://apps/web/components/PromptSelectorModal.tsx)
- [apps/web/components/MarkdownRenderer.tsx](file://apps/web/components/MarkdownRenderer.tsx)
- [apps/web/components/ChatInput.tsx](file://apps/web/components/ChatInput.tsx)
- [apps/web/hooks/useChatStream.ts](file://apps/web/hooks/useChatStream.ts)
- [apps/web/types/chat.ts](file://apps/web/types/chat.ts)
- [apps/web/types/stream.ts](file://apps/web/types/stream.ts)
- [apps/web/types/transfer.ts](file://apps/web/types/transfer.ts)
- [apps/web/app/api/tools/route.ts](file://apps/web/app/api/tools/route.ts)
- [apps/web/app/api/supabase/verify-ownership/route.ts](file://apps/web/app/api/supabase/verify-ownership/route.ts)
- [apps/web/app/api/supabase/delete-conversation/route.ts](file://apps/web/app/api/supabase/delete-conversation/route.ts)
- [apps/web/components/MessageList.tsx](file://apps/web/components/MessageList.tsx)
- [apps/web/components/MessageItem.tsx](file://apps/web/components/MessageItem.tsx)
- [docs/architect/prompt-management-architecture.md](file://docs/architect/prompt-management-architecture.md)
</cite>

## 更新摘要
**变更内容**
- 更新了提示词模板数量统计（从配置文件中发现18个模板）
- 新增了MarkdownRenderer.tsx组件的详细分析
- 完善了架构文档的引用和说明
- 更新了核心组件的完整列表
- 增强了配置文件结构的详细说明

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

提示管理系统是 Web3 AI Agent 项目中的核心功能模块，负责管理和提供预定义的提示词模板。该系统通过统一的配置文件管理各种场景的提示词，包括价格查询、余额查询、Gas 查询、Token 查询和转账操作等，为用户提供便捷的快速提示词选择功能。

系统采用模块化设计，将提示词模板集中管理，支持按分类组织和动态筛选，同时提供了完整的前端交互界面和后端工具调用支持。目前系统包含18个精心设计的提示词模板，覆盖Web3应用的核心场景。

**更新** 系统现已完整交付，包括三个核心组件的实现和完整的配置文件结构。

## 项目结构

提示管理系统主要分布在以下目录结构中：

```mermaid
graph TB
subgraph "提示管理系统架构"
Config[配置层<br/>prompts.ts]
UI[用户界面层<br/>PromptSelector.tsx<br/>PromptSelectorModal.tsx<br/>MarkdownRenderer.tsx<br/>ChatInput.tsx]
Types[类型定义层<br/>chat.ts<br/>stream.ts<br/>transfer.ts]
API[API 层<br/>tools/route.ts<br/>supabase/route.ts]
Storage[存储层<br/>Supabase 数据库]
end
Config --> UI
UI --> API
API --> Storage
Types --> UI
Types --> API
```

**图表来源**
- [apps/web/config/prompts.ts:1-266](file://apps/web/config/prompts.ts#L1-L266)
- [apps/web/components/PromptSelector.tsx:1-77](file://apps/web/components/PromptSelector.tsx#L1-L77)
- [apps/web/app/api/tools/route.ts:1-65](file://apps/web/app/api/tools/route.ts#L1-L65)

**章节来源**
- [apps/web/config/prompts.ts:1-266](file://apps/web/config/prompts.ts#L1-L266)
- [apps/web/components/PromptSelector.tsx:1-77](file://apps/web/components/PromptSelector.tsx#L1-L77)
- [apps/web/components/PromptSelectorModal.tsx:1-109](file://apps/web/components/PromptSelectorModal.tsx#L1-L109)
- [docs/architect/prompt-management-architecture.md:1-543](file://docs/architect/prompt-management-architecture.md#L1-L543)

## 核心组件

### 提示词配置管理

系统的核心是集中化的提示词配置文件，采用 TypeScript 类型安全的设计：

- **提示词模板接口**: 定义了标准的提示词结构，包含唯一标识符、分类、标题、内容和描述
- **分类系统**: 支持价格查询、余额查询、Gas 查询、Token 查询、转账操作和系统提示词六种分类
- **辅助函数**: 提供按分类获取、按ID查找和获取所有分类元数据的功能
- **系统提示词**: 集成了完整的AI行为定义和安全边界说明

### 用户界面组件

系统提供了完整的用户界面交互：

- **提示词选择器**: 展示按分类组织的提示词列表，支持快速选择
- **模态框组件**: 提供响应式的弹窗界面，支持桌面端居中显示和移动端底部抽屉
- **聊天输入框**: 集成提示词选择功能，支持快捷提示词使用
- **Markdown渲染器**: 支持富文本渲染，包括表格、代码块、图片等格式

**更新** 新增了MarkdownRenderer.tsx组件，提供专业的Markdown渲染功能。

**章节来源**
- [apps/web/config/prompts.ts:15-266](file://apps/web/config/prompts.ts#L15-L266)
- [apps/web/components/PromptSelector.tsx:9-77](file://apps/web/components/PromptSelector.tsx#L9-L77)
- [apps/web/components/PromptSelectorModal.tsx:13-109](file://apps/web/components/PromptSelectorModal.tsx#L13-L109)
- [apps/web/components/MarkdownRenderer.tsx:1-160](file://apps/web/components/MarkdownRenderer.tsx#L1-L160)

## 架构概览

提示管理系统采用分层架构设计，各层职责清晰分离：

```mermaid
sequenceDiagram
participant User as 用户
participant ChatInput as 聊天输入框
participant Modal as 提示词模态框
participant Selector as 提示词选择器
participant Config as 配置管理
participant API as 工具 API
User->>ChatInput : 点击"快捷提示词"
ChatInput->>Modal : 打开模态框
Modal->>Selector : 渲染提示词列表
Selector->>Config : 获取分类和提示词
User->>Selector : 选择提示词
Selector->>ChatInput : 设置提示词内容
ChatInput->>API : 发送消息请求
API->>API : 调用相应工具
API-->>ChatInput : 返回处理结果
ChatInput-->>User : 显示响应
```

**图表来源**
- [apps/web/components/ChatInput.tsx:12-119](file://apps/web/components/ChatInput.tsx#L12-L119)
- [apps/web/components/PromptSelectorModal.tsx:13-109](file://apps/web/components/PromptSelectorModal.tsx#L13-L109)
- [apps/web/app/api/tools/route.ts:10-65](file://apps/web/app/api/tools/route.ts#L10-L65)

系统架构特点：

1. **配置驱动**: 所有提示词通过单一配置文件管理，便于维护和扩展
2. **类型安全**: 使用 TypeScript 确保开发时的类型检查和运行时的安全性
3. **模块化设计**: 各组件职责明确，易于测试和维护
4. **响应式界面**: 支持桌面端和移动端的不同交互模式
5. **富文本支持**: 通过MarkdownRenderer提供专业的文本渲染能力

## 详细组件分析

### 提示词配置系统

#### 数据结构设计

```mermaid
classDiagram
class PromptTemplate {
+string id
+PromptCategory category
+string title
+string content
+string description
}
class PromptCategory {
<<enumeration>>
price
balance
gas
token
transfer
system
}
class CategoryMeta {
+string icon
+string label
}
PromptTemplate --> PromptCategory : "使用"
CategoryMeta --> PromptCategory : "映射"
```

**图表来源**
- [apps/web/config/prompts.ts:7-237](file://apps/web/config/prompts.ts#L7-L237)

系统采用枚举类型的分类方式，确保分类的完整性和一致性。每个提示词模板包含：

- **唯一标识符**: 用于精确匹配和引用
- **分类属性**: 支持按功能领域组织
- **用户可见信息**: 标题和描述提升用户体验
- **实际内容**: 机器可执行的提示词模板

#### 提示词模板统计

系统目前包含18个精心设计的提示词模板，分布如下：

- **价格查询**: 5个模板（ETH、BTC、SOL、MATIC、BNB）
- **余额查询**: 4个模板（ETH、Polygon、BSC和指定地址查询）
- **Gas查询**: 3个模板（Ethereum、Polygon、BSC）
- **Token查询**: 4个模板（USDT、USDC及其余额查询）
- **转账操作**: 3个模板（ETH、USDT、USDC转账）

#### 辅助函数实现

系统提供了三个核心辅助函数：

1. **按分类获取**: `getPromptsByCategory()` 支持快速筛选特定功能领域的提示词
2. **按ID查找**: `getPromptById()` 提供精确的提示词检索
3. **分类元数据**: `getAllCategories()` 返回分类的图标和标签信息

**章节来源**
- [apps/web/config/prompts.ts:242-266](file://apps/web/config/prompts.ts#L242-L266)

### 用户界面组件

#### 提示词选择器组件

```mermaid
flowchart TD
Start([组件初始化]) --> LoadCategories["加载分类元数据"]
LoadCategories --> FilterSystem{"过滤 system 分类"}
FilterSystem --> GetPrompts["按分类获取提示词"]
GetPrompts --> CheckEmpty{"提示词为空？"}
CheckEmpty --> |是| SkipCategory["跳过该分类"]
CheckEmpty --> |否| RenderItems["渲染提示词项"]
RenderItems --> AddButton["添加使用按钮"]
AddButton --> Complete([渲染完成])
SkipCategory --> NextCategory["处理下一个分类"]
NextCategory --> FilterSystem
```

**图表来源**
- [apps/web/components/PromptSelector.tsx:9-77](file://apps/web/components/PromptSelector.tsx#L9-L77)

组件特性：

- **响应式布局**: 支持不同屏幕尺寸的适配
- **交互反馈**: 提供悬停效果和过渡动画
- **无障碍设计**: 包含适当的标题和描述信息

#### 模态框组件

```mermaid
stateDiagram-v2
[*] --> Closed
Closed --> Opening : 打开模态框
Opening --> Open : 动画完成
Open --> Closing : 关闭模态框
Closing --> Closed : 动画完成
Open --> EscPressed : 按下 ESC 键
EscPressed --> Closing
```

**图表来源**
- [apps/web/components/PromptSelectorModal.tsx:18-33](file://apps/web/components/PromptSelectorModal.tsx#L18-L33)

模态框实现了完整的生命周期管理：

- **键盘事件处理**: 支持 ESC 键快速关闭
- **动画过渡**: 提供流畅的打开和关闭动画
- **响应式设计**: 桌面端居中显示，移动端底部抽屉

#### Markdown渲染器组件

```mermaid
flowchart TD
Start([组件初始化]) --> ParseMarkdown["解析Markdown内容"]
ParseMarkdown --> ProcessComponents["处理组件映射"]
ProcessComponents --> RenderElements["渲染HTML元素"]
RenderElements --> HandleImages["特殊处理图片"]
HandleImages --> ApplyStyles["应用样式类"]
ApplyStyles --> Complete([渲染完成])
```

**图表来源**
- [apps/web/components/MarkdownRenderer.tsx:12-160](file://apps/web/components/MarkdownRenderer.tsx#L12-L160)

Markdown渲染器提供了丰富的文本渲染功能：

- **基础元素**: 支持段落、标题、列表、加粗、斜体等
- **代码支持**: 行内代码和代码块渲染
- **表格支持**: 完整的表格渲染能力
- **图片处理**: 特殊处理Token Logo图片，支持错误处理
- **链接处理**: 外部链接安全打开
- **样式定制**: 基于CSS变量的主题适配

**章节来源**
- [apps/web/components/PromptSelector.tsx:1-77](file://apps/web/components/PromptSelector.tsx#L1-L77)
- [apps/web/components/PromptSelectorModal.tsx:1-109](file://apps/web/components/PromptSelectorModal.tsx#L1-L109)
- [apps/web/components/MarkdownRenderer.tsx:1-160](file://apps/web/components/MarkdownRenderer.tsx#L1-L160)

### 流式消息处理

#### SSE 流式消费机制

```mermaid
sequenceDiagram
participant Client as 客户端
participant Hook as useChatStream
participant API as /api/chat
participant Server as 服务器
Client->>Hook : sendMessage(messages)
Hook->>API : POST /api/chat
API->>Server : 处理消息
Server-->>API : 返回 SSE 流
API-->>Hook : 流式响应
Hook->>Hook : 解析 SSE 事件
Hook->>Hook : 更新状态
Hook-->>Client : 实时更新内容
Note over Hook : 支持重试机制<br/>最大重试次数 : 2<br/>超时时间 : 30秒
```

**图表来源**
- [apps/web/hooks/useChatStream.ts:184-274](file://apps/web/hooks/useChatStream.ts#L184-L274)

Hook 实现了高级的流式处理功能：

- **节流更新**: 防止频繁的状态更新影响性能
- **错误处理**: 完善的异常捕获和重试机制
- **内存管理**: 使用 ref 和 state 的组合优化性能
- **转账数据**: 支持transfer_data类型的流式传输

#### 工具调用状态管理

```mermaid
flowchart TD
Start([开始工具调用]) --> ParseArgs["解析工具参数"]
ParseArgs --> CreateToolCall["创建工具调用状态"]
CreateToolCall --> AddToList["添加到状态列表"]
AddToList --> UpdateUI["更新用户界面"]
UpdateUI --> Running{"执行中？"}
Running --> |是| WaitComplete["等待完成"]
Running --> |否| Done[完成]
WaitComplete --> CheckStatus{"状态变化？"}
CheckStatus --> |是| UpdateUI
CheckStatus --> |否| WaitComplete
WaitComplete --> Running
```

**图表来源**
- [apps/web/hooks/useChatStream.ts:129-181](file://apps/web/hooks/useChatStream.ts#L129-L181)

**章节来源**
- [apps/web/hooks/useChatStream.ts:1-318](file://apps/web/hooks/useChatStream.ts#L1-L318)

### 类型定义系统

#### 流式输出类型

系统定义了完整的流式通信协议：

```mermaid
erDiagram
StreamChunk {
string type
string content
ToolCall toolCall
TransferData transferData
string error
}
ToolCall {
string id
string name
json arguments
json result
}
ToolCallUIState {
string id
string name
json arguments
string status
json result
}
TransferData {
string id
string from
string to
string tokenSymbol
string tokenAddress
string amount
string chain
string status
string txHash
string error
string estimatedGas
}
Message {
string id
string role
string content
number timestamp
ToolCall toolCalls
boolean isError
TransferData transferData
}
StreamChunk ||--|| ToolCall : "包含"
StreamChunk ||--|| TransferData : "包含"
Message ||--|| ToolCall : "包含"
Message ||--|| TransferData : "包含"
```

**图表来源**
- [apps/web/types/stream.ts:6-37](file://apps/web/types/stream.ts#L6-L37)
- [apps/web/types/chat.ts:3-32](file://apps/web/types/chat.ts#L3-L32)
- [apps/web/types/transfer.ts:7-20](file://apps/web/types/transfer.ts#L7-L20)

**章节来源**
- [apps/web/types/stream.ts:1-37](file://apps/web/types/stream.ts#L1-L37)
- [apps/web/types/chat.ts:1-32](file://apps/web/types/chat.ts#L1-L32)
- [apps/web/types/transfer.ts:1-20](file://apps/web/types/transfer.ts#L1-L20)

## 依赖关系分析

提示管理系统与其他系统组件的依赖关系：

```mermaid
graph TB
subgraph "提示管理系统"
Prompts[提示词配置]
UI[用户界面组件]
Hooks[React Hooks]
Markdown[Markdown渲染]
end
subgraph "外部依赖"
Web3Tools[web3-tools 包]
Supabase[Supabase 数据库]
Wagmi[wagmi 钱包连接]
Endecode[编码解码工具]
end
subgraph "类型系统"
ChatTypes[聊天类型定义]
StreamTypes[流式类型定义]
TransferTypes[转账类型定义]
end
Prompts --> UI
UI --> Hooks
UI --> Markdown
Hooks --> Web3Tools
Hooks --> Supabase
UI --> Wagmi
Hooks --> ChatTypes
Hooks --> StreamTypes
Hooks --> TransferTypes
```

**图表来源**
- [apps/web/app/api/tools/route.ts:2-3](file://apps/web/app/api/tools/route.ts#L2-L3)
- [apps/web/hooks/useChatStream.ts:4-7](file://apps/web/hooks/useChatStream.ts#L4-L7)

### 外部依赖管理

系统对外部依赖的处理策略：

1. **工具调用**: 通过 `/api/tools` 端点调用 web3-tools 包提供的功能
2. **数据持久化**: 使用 Supabase 进行对话历史和用户数据存储
3. **钱包集成**: 通过 wagmi 实现钱包连接和身份验证
4. **文本处理**: 使用react-markdown和remark-gfm进行Markdown渲染

**章节来源**
- [apps/web/app/api/tools/route.ts:1-65](file://apps/web/app/api/tools/route.ts#L1-L65)
- [apps/web/app/api/supabase/verify-ownership/route.ts:1-95](file://apps/web/app/api/supabase/verify-ownership/route.ts#L1-L95)

## 性能考虑

### 内存优化策略

系统采用了多项性能优化措施：

1. **状态管理优化**: 使用 `useRef` 和 `useState` 的组合避免不必要的重渲染
2. **节流机制**: 内容更新采用 50ms 节流，防止频繁的 DOM 操作
3. **缓存策略**: 提示词数据在内存中缓存，避免重复计算
4. **资源清理**: 组件卸载时自动清理定时器和事件监听器
5. **图片优化**: Markdown渲染器对图片进行错误处理和优化

### 网络请求优化

```mermaid
flowchart TD
Request[网络请求] --> Retry{重试机制}
Retry --> |成功| Success[成功响应]
Retry --> |失败| MaxRetries{达到最大重试次数？}
MaxRetries --> |是| Error[错误处理]
MaxRetries --> |否| Delay[1秒延迟后重试]
Delay --> Retry
Success --> Cleanup[清理资源]
Error --> Cleanup
```

**图表来源**
- [apps/web/hooks/useChatStream.ts:195-274](file://apps/web/hooks/useChatStream.ts#L195-L274)

**章节来源**
- [apps/web/hooks/useChatStream.ts:18-318](file://apps/web/hooks/useChatStream.ts#L18-L318)

## 故障排除指南

### 常见问题诊断

#### 提示词加载问题

**症状**: 提示词选择器无法显示任何内容

**可能原因**:
1. 配置文件导入路径错误
2. 分类元数据映射问题
3. 权限不足导致数据访问失败

**解决方案**:
1. 检查 `prompts.ts` 文件的导出语句
2. 验证分类枚举值的一致性
3. 确认组件的权限配置

#### 流式响应问题

**症状**: 消息无法实时显示或显示延迟

**可能原因**:
1. SSE 连接中断
2. 服务器端流式处理异常
3. 客户端解析错误

**解决方案**:
1. 检查网络连接和防火墙设置
2. 验证服务器端的流式响应格式
3. 查看浏览器控制台的错误信息

#### 工具调用失败

**症状**: 提示词执行后没有返回预期结果

**可能原因**:
1. 工具 API 端点不可用
2. 参数格式不正确
3. 外部服务调用失败

**解决方案**:
1. 验证 `/api/tools` 端点的可用性
2. 检查工具参数的 JSON 格式
3. 查看外部服务的响应状态

#### Markdown渲染问题

**症状**: Markdown内容无法正确渲染

**可能原因**:
1. Markdown语法错误
2. 图片URL无效
3. 样式冲突

**解决方案**:
1. 验证Markdown语法的正确性
2. 检查图片URL的有效性
3. 确认CSS变量的正确应用

**章节来源**
- [apps/web/hooks/useChatStream.ts:243-274](file://apps/web/hooks/useChatStream.ts#L243-L274)
- [apps/web/app/api/tools/route.ts:54-64](file://apps/web/app/api/tools/route.ts#L54-L64)
- [apps/web/components/MarkdownRenderer.tsx:114-152](file://apps/web/components/MarkdownRenderer.tsx#L114-L152)

## 结论

提示管理系统通过模块化的设计和类型安全的实现，为 Web3 AI Agent 提供了强大而灵活的提示词管理功能。系统的主要优势包括：

1. **统一管理**: 集中的配置文件简化了提示词的维护和扩展
2. **类型安全**: TypeScript 提供了完整的编译时类型检查
3. **用户体验**: 响应式的界面设计支持多种设备和交互方式
4. **性能优化**: 多层次的优化策略确保了良好的用户体验
5. **可扩展性**: 清晰的架构设计便于未来功能的添加和修改
6. **富文本支持**: 专业的Markdown渲染能力提升了内容展示质量
7. **完整的功能集**: 18个提示词模板覆盖Web3应用的核心场景

该系统为 Web3 应用场景下的智能代理提供了坚实的基础，通过预定义的提示词模板帮助用户更高效地进行区块链相关的查询和操作。随着项目的不断发展，提示管理系统将继续演进以满足更多样化的业务需求。

**更新** 系统现已完整交付，包括PromptSelector.tsx、PromptSelectorModal.tsx、MarkdownRenderer.tsx三个核心组件的实现，18个提示词模板（5个分类），以及完整的配置文件结构，为Web3 AI Agent提供了完整的提示词管理解决方案。