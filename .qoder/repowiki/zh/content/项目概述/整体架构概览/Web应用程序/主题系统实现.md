...
## 更新摘要
**变更内容**
- 主题系统已完全重构为基于CSS变量的新系统
- 新增服务器端渲染脚本注入机制，消除UI闪烁问题
- 改进主题初始化逻辑，实现同步初始化和SSR兼容性
- 新增getThemeInitScript函数，提供统一的主题初始化脚本
- 新增暗色模式自动同步功能
- 实现系统主题跟随机制
- 全面的UI组件适配，包括滚动条、代码块、光标动画、文本选择
- 集成RainbowKit组件的主题一致性
- 优化主题切换的性能和用户体验
- **更新** MessageList组件布局结构调整，padding从外层容器移动到内部包装div，改进响应式设计和主题系统应用效果
...
**更新** 最重要的改进是新增了服务器端渲染脚本注入机制，通过在`layout.tsx`中添加内联脚本，在React执行之前设置主题，彻底消除了UI闪烁问题。同时，`ThemeProvider`组件实现了同步初始化逻辑，确保主题状态在客户端渲染时能够立即生效。
...
- [page.tsx:350-376](file://apps/web/app/page.tsx#L350-L376)

**章节来源**
- [layout.tsx:1-59](file://apps/web/app/layout.tsx#L1-L59)
...
**更新** 主题提供者现在同时管理`data-theme`属性和`dark`类名，确保与Tailwind CSS的darkMode功能兼容。通过同步初始化逻辑，主题状态能够在客户端渲染时立即生效，配合服务器端脚本注入，彻底消除了UI闪烁问题。
...
**章节来源**
- [MessageList.tsx:1-61](file://apps/web/components/MessageList.tsx#L1-L61)
...
**章节来源**
- [layout.tsx:17-59](file://apps/web/app/layout.tsx#L17-L59)
- [ThemeProvider.tsx:12-28](file://apps/web/lib/theme/ThemeProvider.tsx#L12-L28)
- [ThemeProvider.tsx:48-62](file://apps/web/lib/theme/ThemeProvider.tsx#L48-L62)
...
9. **初始化优化**：**更新** 同步主题初始化确保客户端渲染时的即时响应
...
**更新** 最重要的改进是SSR闪烁消除机制的实现，通过服务器端脚本注入和同步初始化逻辑，主题系统现在能够在服务器端和客户端都提供一致且无闪烁的主题体验。这标志着主题系统达到了生产环境级别的成熟度和可靠性。